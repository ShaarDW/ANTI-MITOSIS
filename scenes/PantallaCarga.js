export default class PantallaCarga extends Phaser.Scene {
    ...
    preload() {
        // Cargar el sprite de la célula para la animación de carga
        this.load.spritesheet("celula", "public/assets/CELULAS-SPRITESHEET.png", { frameWidth: 32, frameHeight: 32 });

         // ...agrega aquí todos los assets necesarios
        // Crear la célula y animarla en movimiento circular
    ...
    create() {
        ...
        // Cuando termina la carga, espera un segundo y cambia de escena
        this.load.on('complete', () => {
    ...
    update(time, delta) {
        ...
        // Cuando termine la carga, pasar a la siguiente escena
        if (this.load.totalComplete === this.load.totalToLoad) {
            this.scene.start('MenuPrincipalScene'); // Cambia esto por el nombre real de tu escena principal
        }
    }
}